/**
 * SQL execution engine — safe read-only SQLite query runner.
 *
 * Creates an isolated in-memory SQLite database from the active story's seed
 * data. Each story gets its own database instance — no cross-story data.
 *
 * Uses sql.js (pure WASM SQLite) to avoid native addon crashes.
 */
...
async function getSqlModule() {
  if (!_sqlModulePromise) {
    _sqlModulePromise = initSqlJs({ locateFile: () => WASM_PATH });
  }
  return _sqlModulePromise;
}

/** Cache of story databases: storyId -> Database. */
const _dbCache = new Map<string, Database>();

/**
 * Create (or return a cached) in-memory SQLite database for a story.
 * The schema is shared across all stories; the seed data is story-specific.
 */
export async function getStoryDatabase(storyId: string): Promise<Database> {
...
/** Destroy a story's cached database (used when switching stories). */
...
// ---------- Validation (unchanged — shared across stories) ----------

/** Strip SQL comments and normalise whitespace for validation. Keeps string literals intact. */
...
export function validateQuery(rawSql: string): ValidationResult {
...
// ---------- Schema metadata (shared across stories) ----------
...
export async function getSchema(storyId: string): Promise<TableInfo[]> {
  const db = await getStoryDatabase(storyId);
  const tables = ["suspects", "employees", "locations", "visits", "calls", "transactions", "messages", "security_logs", "evidence"];
  return tables.map((t) => {
    ...
    return {
      ...
      columns: colsRaw.map((c) => ({
        ...
        notnull: c.notnull,
        pk: c.pk,
        description: COLUMN_DESCRIPTIONS[t]?.[c.name] ?? "",
      })),
    };
  });
}
