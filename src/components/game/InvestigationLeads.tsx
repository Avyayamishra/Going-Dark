...
import { Compass, CornerDownRight, Code2 } from "lucide-react";
...
export function InvestigationLeads({ onRunQuery, onRunPython }: InvestigationLeadsProps) {
  ...
  return (
    <div className="p-2 space-y-2">
      ...
      <p className="px-1 text-[10px] text-muted-foreground/70 leading-snug">
        Investigate any thread in any order. Use a starter query or starter code, or write your own.
      </p>
      <div className="space-y-1.5">
        {leads.map((lead) => {
          const language = lead.language ?? "sql";
          const starter = language === "python" ? lead.starterCode : lead.starterQuery;
          const canLoad = language === "python" ? !!onRunPython : !!onRunQuery;
          return (
            <div key={lead.id} className="border border-border/50 bg-black/20 rounded-sm p-2 hover:border-primary/30 transition-colors">
              ...
              {starter && canLoad && (
                <button
                  onClick={() => {
                    play("click");
                    if (language === "python") onRunPython?.(starter);
                    else onRunQuery?.(starter);
                  }}
                  className={cn(
                    "mt-1.5 inline-flex items-center gap-1 text-[9px] font-mono uppercase tracking-wider transition-colors border-b",
                    language === "python"
                      ? "text-primary/80 hover:text-primary border-primary/30 hover:border-primary/60"
                      : "text-primary/80 hover:text-primary border-primary/30 hover:border-primary/60",
                  )}
                >
                  <CornerDownRight className="size-2.5" />
                  {language === "python" ? "Load starter code" : "Load starter query"}
                </button>
              )}
            </div>
          );
