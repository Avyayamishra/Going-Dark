...
import { Target, CheckCircle2, Circle, CornerDownRight } from "lucide-react";
...
export function InvestigationProgress({ onLoadQuery }: InvestigationProgressProps) {
  ...
  const handleLoadStarterQuery = (sql: string, objectiveId: string) => {
    play("click");
    penalizeScore(100);
    toast({ title: "Lead loaded", description: `-100 points. A search lead has been loaded for ${objectiveId}.` });
    onLoadQuery?.(sql);
  };
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {o.starterQuery && onLoadQuery && (
                        <button onClick={() => handleLoadStarterQuery(o.starterQuery!, o.id)} className="mt-1.5 inline-flex items-center gap-1 text-[9px] font-mono uppercase tracking-wider text-amber-400/80 hover:text-amber-400 border-b border-amber-400/30 hover:border-amber-400/60 transition-colors cursor-pointer">
