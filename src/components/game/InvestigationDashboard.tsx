...
import { analyzeQuery, type SqlConcept } from "@/lib/sql/analyzer";
import {
  FolderOpen,
  Users,
...
import { SqlInsight } from "@/components/game/SqlInsight";
import { EvidenceToast, type EvidenceNotification } from "@/components/game/EvidenceToast";
import { SqlTutorChatbot } from "@/components/game/SqlTutorChatbot";
import { Sheet, SheetContent, SheetHeader, SheetTitle } from "@/components/ui/sheet";
import { Button } from "@/components/ui/button";
import { useGameStore, useActiveStory, useDiscoveredEvidence, useShownInsights } from "@/lib/game/store";
...
export function InvestigationDashboard() {
  const [sql, setSql] = useState<string>("SELECT * FROM suspects;");
  ...
  const [insight, setInsight] = useState<{ concept: SqlConcept; title: string; body: string } | null>(null);
  ...
  const shownInsights = useShownInsights();
  const { play } = useAudio();
  ...
  const handleResult = useCallback(
    (r: QueryResult) => {
      setResult(r);
      setRunning(false);
      // Surface a contextual SQL insight if the query introduced a new concept.
      if (r.ok && sql) {
        const a = analyzeQuery(sql);
        if (a.insight && !shownInsights.includes(a.insight.concept)) {
          setInsight(a.insight);
        }
      }
    },
    [sql, shownInsights],
  ...
  // Watch discoveredEvidence for new items -> show the cinematic toast.
  // The setState call is inside a condition that only fires when the count
  // genuinely increased, so there is no cascading render.
  ...
  useEffect(() => {
    ...
    if (!latest) return;
    // Defer the toast to a microtask to avoid a synchronous setState-in-effect.
    const id = setTimeout(() => {
      setEvidenceNotif({
        id: latest.id,
        name: latest.name,
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        <section className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="flex-1 min-h-0">
            <PanelGroup direction="vertical" autoSaveId="sql-editor-results">
              ...
              <Panel defaultSize={58} minSize={20} className="min-h-0">
                <div className="h-full bg-background/40">
                  <QueryResults
                    ...
                    running={running}
                    insight={insight}
                    onInsightDismiss={() => setInsight(null)}
                  />
                </div>
              </Panel>
            </PanelGroup>
      ...
      {/* Evidence discovery cinematic notification */}
      <EvidenceToast evidence={evidenceNotif} onDismiss={() => setEvidenceNotif(null)} />

      {/* Draggable SQL Tutor chatbot */}
      <SqlTutorChatbot />
    </div>
  );
}
        ...
        {tabs.map((t) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex-1 flex flex-col items-center gap-0.5 py-2 transition-colors group",
                isActive
...
function LeftContent({
  ...
  if (tab === "database") {
    return <DatabaseExplorer onInsertQuery={onInsertQuery} />;
  }
  if (tab === "notes") {
...
function RightContent({ tab, onLeadQuery }: { tab: RightTab; onLeadQuery: (sql: string) => void }) {
  ...
  if (tab === "leads") {
    return (
      <div className="h-full overflow-auto">
        <InvestigationLeads onRunQuery={onLeadQuery} />
      </div>
    );
  }
  if (tab === "progress") {
    return (
      <div className="h-full overflow-auto">
        <InvestigationProgress onLoadQuery={(q) => onLeadQuery(q)} />
      </div>
    );
  }
  return (
...
function ObjectiveBanner() {
  ...
  const completed = useGameStore((s) => {
    if (!s.activeStoryId) return [] as string[];
